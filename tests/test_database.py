...
def test_db_decorator_connects_and_closes(tmp_path: Path) -> None:
    ...
    class MyDB(fl.DataBase):
        users: fl.Table = fl.Table(schema=UserSchema)
...
def test_schema_inheritance_and_table_interaction(tmp_path: Path) -> None:
    ...
    class DerivedS(BaseS):
        b: fl.String = fl.String()

    class MyDB(fl.DataBase):
        t: fl.Table = fl.Table(schema=DerivedS)
...
def test_db_multiple_tables_single_transaction(tmp_path: Path) -> None:
    ...
    class OrderSchema(fl.Schema):
        id: fl.Int64 = fl.Int64(primary_key=True)
        user_id: fl.Int64 = fl.Int64()
        product: fl.String = fl.String()

    class MyDB(fl.DataBase):
        users: fl.Table = fl.Table(schema=UserSchema)
        orders: fl.Table = fl.Table(schema=OrderSchema)
...
def test_db_connection_error_propagation(tmp_path: Path) -> None:
    ...
    def failing_func() -> None:
        _ = Project.db.t.create_or_replace().insert_into(pl.DataFrame({"id": [1]}))
...
def test_db_sql_execution_within_context(tmp_path: Path) -> None:
    ...
    class S(fl.Schema):
        x: fl.Int64 = fl.Int64()
        y: fl.Float64 = fl.Float64()

    class MyDB(fl.DataBase):
        data: fl.Table = fl.Table(schema=S)
    ...
    with Project.db:
        ...
        _ = Project.db.data.create_or_replace().insert_into(df)
...
def test_db_show_tables_reflects_schema(tmp_path: Path) -> None:
    ...
    class MyDB(fl.DataBase):
        table_one: fl.Table = fl.Table(schema=S1)
        table_two: fl.Table = fl.Table(schema=S2)
    ...
    with Project.db:
        _ = Project.db.table_one.create_or_replace().insert_into(
            pl.DataFrame({"a": [1]})
        )
        _ = Project.db.table_two.create_or_replace().insert_into(
            pl.DataFrame({"b": ["x"]})
        )
...
def test_schema_composite_pk_duckdb_integration(tmp_path: Path) -> None:
    ...
    with Project.db:
        _ = Project.db.t.create_or_replace().insert_into(df1)
...
def test_unique_constraint_duckdb_integration(tmp_path: Path) -> None:
    ...
    with Project.db:
        ...
        with pytest.raises(duckdb.ConstraintException):
            _ = Project.db.t.insert_into(df2)
